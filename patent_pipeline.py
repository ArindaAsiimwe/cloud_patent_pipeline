...
# KAGGLE PATHS
DATA_FOLDER = "/kaggle/input/datasets/arindaasiimwe/clouddata"
OUTPUT_FOLDER = "/kaggle/working/output"
DB_PATH = "/kaggle/working/database/patents.db"


Path(OUTPUT_FOLDER).mkdir(exist_ok=True, parents=True)
Path("/kaggle/working/database").mkdir(exist_ok=True, parents=True)

# DEBUG: List available files
print("DEBUG: Checking available files in input directory...")
import os
if os.path.exists(DATA_FOLDER):
    print(f"✓ Found {DATA_FOLDER}")
    for file in os.listdir(DATA_FOLDER):
        print(f"  - {file}")
else:
    print(f"✗ {DATA_FOLDER} not found!")
...
print(f"\n[1/3] Loading g_patent.tsv (all records)...")
try:
...
# Load abstracts
print(f"\n[2/3] Loading g_patent_abstract.tsv...")

# Convert patent_ids to set for O(1) lookup
patent_ids_set = set(patents_df['patent_id'].values)
...
try:
    ...
    abstract_df = abstract_df[abstract_df['patent_id'].isin(patent_ids_set)]
...
# Load company data
print("\n[3/3] Loading g_persistent_assignee.tsv...")
...
try:
    ...
    assignee_df = assignee_df[assignee_df['patent_id'].isin(patent_ids_set)]
...
# Use dictionary for O(1) inventor lookup instead of list search (O(n))
inventor_name_to_id = {}  # Maps name -> inventor_id for fast lookup
inventors_data = []
...
for patent_id in patent_ids:
    ...
    for _ in range(num_inventors):
        ...
        # OPTIMIZED: Use dict lookup instead of list search (O(1) vs O(n))
        if inventor_name in inventor_name_to_id:
            inventor_id = inventor_name_to_id[inventor_name]
        else:
            inventor_id = inventor_id_counter
            inventor_name_to_id[inventor_name] = inventor_id
    ...
    # Progress indicator
    patent_idx += 1
    if patent_idx % 100000 == 0:
        print(f"      Creating inventors... {patent_idx:,} patents processed", end='\r')
...
patents_clean.to_csv(f"{OUTPUT_FOLDER}/clean_patents.csv", index=False)
print(f"      ✓ Exported")

print("[2/3] Exporting clean_inventors.csv...")
inventors_df.to_csv(f"{OUTPUT_FOLDER}/clean_inventors.csv", index=False)
print(f"      ✓ Exported")
...
if not assignee_clean.empty:
    ...
    companies_unique.to_csv(f"{OUTPUT_FOLDER}/clean_companies.csv", index=False)
    print(f"      ✓ Exported")
else:
